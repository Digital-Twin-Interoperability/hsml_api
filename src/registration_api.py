...
def send_kafka_message(topic: str, message: dict):
    """Publish a message to a Kafka topic (as JSON), if Kafka is configured."""
    if not producer:
        return
    try:
        ...
        # Log any failures in sending Kafka message
...
async def register_user(data: dict):
    ...
    # Generate a unique DID and private key for the new user
    try:
...
async def register_entity_endpoint(payload: dict):
    ...
    # If the entity is an Agent, create a Kafka topic and send a welcome message
    topic_name = None
    ...
    # If the entity is a Credential, handle special verification for authorized domain access
...
# Helper: generate_did_key uses CLItool to produce a DID and private key
def generate_did_key():
    """Generate a unique DID:key and return (did, private_key_pem)."""
    # This uses an external CLI tool. In a production environment, replace with a proper DID library call.
    ...
    # Read the private key from the file written by CLItool (assumed "private_key.pem" in current directory)
    try:
        with open("private_key.pem", "r") as key_file:
            private_key_pem = key_file.read()
    except FileNotFoundError:
        raise RuntimeError("Private key file not found after DID generation.")
    # Remove the private key file for security after reading it
    try:
